...
import type { AdminEmergencyCreditOrderListItem } from "@/types"
...
type RefundTarget = {
  ...
  creditCloseReason?: string | null
  requiresForce: boolean
  creditOwnsCycle: boolean
}
...
function creditStatusLabel(status: string | null | undefined) {
  if (!status) return "—"
  return CREDIT_STATUS_LABELS[status] || status
}

function formatCreditCell(item: AdminEmergencyCreditOrderListItem) {
  if (!item.credit_status) return "—"
  const status = creditStatusLabel(item.credit_status)
  if (
    item.credit_status === "inactive" &&
    item.credit_close_reason
  ) {
    const reason =
      CLOSE_REASON_LABELS[item.credit_close_reason] || item.credit_close_reason
    return `${status} · ${reason}`
  }
  return status
}

interface AdminEmergencyOrdersTabProps {
  onRefundSuccess?: (message: string) => void
}

export function AdminEmergencyOrdersTab({
  onRefundSuccess,
}: AdminEmergencyOrdersTabProps) {
  const [items, setItems] = useState<AdminEmergencyCreditOrderListItem[]>([])
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await adminApi.emergencyOrders.list({
        limit: LIMIT,
        offset: (page - 1) * LIMIT,
  ...
  function openRefund(item: AdminEmergencyCreditOrderListItem) {
    setRefundTarget({
      orderId: item.id,
      ...
      creditCloseReason: item.credit_close_reason,
      requiresForce: item.requires_force,
      creditOwnsCycle: item.credit_owns_cycle,
    })
    setRefundReason("")
  ...
  async function confirmRefund() {
    ...
    if (refundTarget.requiresForce && !refundForce) {
      setRefundError(
        "Esta emergencia ya fue atendida. Marca “Forzar reembolso” para continuar.",
      )
      return
    }
    ...
    setRefundError(null)
    try {
      const res = await adminApi.emergencyOrders.refund(refundTarget.orderId, {
        reason,
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500 max-w-2xl">
        Compras de crédito de emergencia vía Nuvei. Desde aquí puedes reembolsar
        el pago; si el crédito sigue abierto, se cierra para que no puedan
        llamar.
      </p>
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[200px] flex-1 max-w-md space-y-1">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Usuario, email, TX o UUID…"
                ...
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/80">
                    ...
                    <td className="px-4 py-3">
                      {item.credit_status ? (
                        <span
                          className={`inline-flex rounded px-2 py-0.5 text-xs font-medium ${CREDIT_STATUS_STYLES[item.credit_status] || "bg-gray-100 text-gray-600"}`}
                        >
                          {formatCreditCell(item)}
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      {item.nuvei_transaction_id ? (
                        <div className="text-[11px] font-mono text-gray-400 truncate max-w-[160px]">
                          {item.nuvei_transaction_id}
                        </div>
                      ) : (
                        <span className="text-xs text-gray-400">—</span>
              ...
              >
                <div className="flex justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{item.buyer_email}</p>
                  </div>
                  <span
                ...
                <p className="text-sm text-gray-700">
                  ${Number(item.amount_usd).toFixed(2)} · {formatCreditCell(item)}
                </p>
                {item.refundable && (
                  <Button
                    variant="outline"
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Reembolsar crédito de emergencia</DialogTitle>
                ...
                {refundTarget ? (
                  <>
                    ...
                    <p className="text-xs">
                      Crédito:{" "}
                      <strong>
                        {creditStatusLabel(refundTarget.creditStatus)}
                        {refundTarget.creditCloseReason
                          ? ` · ${CLOSE_REASON_LABELS[refundTarget.creditCloseReason] || refundTarget.creditCloseReason}`
                          : ""}
                      </strong>
                    </p>
                    {refundTarget.creditStatus === "calling" &&
                      refundTarget.creditOwnsCycle && (
                      <p className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                        Hay una llamada en curso. Al reembolsar, el crédito se
                        cerrará y el caso dejará de aparecer en atención.
                      </p>
                    )}
                    {!refundTarget.creditOwnsCycle &&
                      (refundTarget.creditStatus === "active" ||
                        refundTarget.creditStatus === "calling") && (
                        <p className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900">
                          El usuario tiene un crédito activo de una compra más
                          reciente. Este reembolso es histórico: se devolverá el
                          pago de esta orden pero no se cerrará el acceso actual.
                        </p>
                      )}
                    <div className="space-y-1.5">
                      <label
                        htmlFor="emergency-refund-reason"
                        className="text-xs font-medium text-gray-700 block"
                      >
                        Motivo del reembolso (obligatorio)
                      </label>
                      <Textarea
                        id="emergency-refund-reason"
                        value={refundReason}
                        onChange={(e) => setRefundReason(e.target.value)}
                        placeholder="Ej. El usuario compró por error…"
                        rows={3}
                        maxLength={500}
                    ...
                    {refundTarget.requiresForce && (
                      <label className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900 cursor-pointer">
                        ...
                        />
                        <span>
                          <strong>Forzar reembolso:</strong> la emergencia ya
                          fue marcada como atendida. Confirma que quieres
                          reembolsar de todos modos.
                        </span>
                      </label>
                    )}
                  </>
